...
const TOGGLES = [
  { key: "locationShare", label: "Share Live Location", sub: "Let trusted contacts track your ride" },
  { key: "sos",           label: "Emergency SOS",       sub: "One-tap alert to contacts & police"   },
  { key: "tripShare",     label: "Trip Sharing",        sub: "Auto-share trip details via WhatsApp"  },
] as const;

type ToggleKey = (typeof TOGGLES)[number]["key"];

export function PrivacySettings() {
  const [prefs, setPrefs] = useState<Record<ToggleKey, boolean>>({ locationShare: true, sos: true, tripShare: false });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-5 shadow-sm">
        <p className="text-[13px] font-bold text-[var(--color-text-primary)] mb-3 flex items-center gap-2">
          <IconShield size={15} className="text-[var(--color-primary)]" /> Emergency Contacts
        </p>
        {EMERGENCY_CONTACTS.map((c) => (
          <div key={c} className="flex items-center gap-3 py-2.5 border-b last:border-0 border-[var(--color-border)]">
            <Avatar size="sm">
              <Avatar.Fallback className="bg-[var(--color-primary-light)] text-[var(--color-primary)] font-bold text-[11px]">
                {c.slice(0, 2).toUpperCase()}
              </Avatar.Fallback>
            </Avatar>
            <span className="flex-1 text-[13px] text-[var(--color-text-secondary)]">{c}</span>
            <button className="text-[12px] text-[var(--color-danger)] font-semibold">Remove</button>
          </div>
        ))}
