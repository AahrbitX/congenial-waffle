...
export function toBackendVehicleType(
  type: string,
): "hatchback" | "sedan" | "suv" | "minivan" {
  const t = type.toLowerCase();
  if (
    t.includes("tempo") ||
    t.includes("traveller") ||
    t.includes("mini bus") ||
    t.includes("minibus")
  )
    return "minivan";
...
export async function getVehicles(): Promise<Vehicle[]> {
  const res = await request<{ success: boolean; data: VehicleApiRow[] }>(
    "/api/vehicles",
  );
...
export async function createBooking(
  ...
  }>("/api/bookings/create", { method: "POST", body: JSON.stringify(payload) });
...
export async function cancelBooking(bookingId: string): Promise<void> {
  await request<{ success: boolean }>(`/api/bookings/${bookingId}/cancel`, {
    method: "PATCH",
  });
}
