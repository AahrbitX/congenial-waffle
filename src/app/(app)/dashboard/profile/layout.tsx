      ...
      {isSubScreen && subTitle && (
        <div className="flex items-center gap-3">
          <Link
            ...
            className="flex items-center gap-1.5 text-[13px] font-semibold text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] border border-[var(--color-border)] px-3 py-1.5 rounded-xl hover:bg-[var(--color-surface-muted)] transition-all"
          >
            <IconChevronLeft size={14} /> Back
          </Link>
          <p className="text-[16px] font-black text-[var(--color-text-primary)]">{subTitle}</p>
        </div>
      )}
      {children}
    </div>
