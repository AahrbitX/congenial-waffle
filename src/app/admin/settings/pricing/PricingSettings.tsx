...
type PricingState = {
  ...
  defaultUnit: FareUnit;
  rows: ServiceRow[];
};

function toUnit(u: string): FareUnit {
  return (FARE_UNITS as readonly string[]).includes(u)
    ? (u as FareUnit)
    : "per km";
}
...
const BLANK_PRICING: PricingState = {
  defaultAmount: 0,
  defaultUnit: "per km",
  rows: [],
};
          ...
          {[...Array(9)].map((_, i) => (
            <div key={i} className="px-3 py-3 rounded-xl space-y-2">
              <div className="flex items-center gap-2.5">
                ...
                <Skeleton
                  className={`h-3.5 rounded ${i % 3 === 0 ? "w-28" : i % 3 === 1 ? "w-36" : "w-32"}`}
                />
              </div>
...
export default function PricingSettings({ isLoading }: Props) {
  ...
  useEffect(() => {
    ...
    const dbMap = Object.fromEntries(
      (dbPricing ?? []).map((p) => [p.vehicleType, p]),
    );
    setPricingData(
      fleetVehicles.map((v: FleetVehicle) => {
        ...
        return {
          ...
          rows: Object.entries(
            db.serviceFares as Record<string, { amount: number; unit: string }>,
          ).map(([key, fare]) => ({
            key,
            amount: fare.amount,
            unit: toUnit(fare.unit),
          })),
        };
      }),
    );
  ...
  const setDefault = (
    field: "defaultAmount" | "defaultUnit",
    value: number | FareUnit,
  ) =>
    setPricingData((p) =>
      p.map((s, i) => (i === selectedIdx ? { ...s, [field]: value } : s)),
    );
      ...
      p.map((s, i) =>
        i === selectedIdx
          ? {
              ...
              rows: [
                ...s.rows,
                { key: "", amount: 0, unit: "per km" as FareUnit },
              ],
            }
          : s,
      ),
      ...
      {showSkeleton ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-border text-center">
          ...
          <p className="text-xs text-text-tertiary mt-1">
            Add vehicles in Fleet Settings first.
          </p>
        </div>
      ) : (
        <div className="flex flex-col md:flex-row rounded-2xl border border-border overflow-hidden bg-surface">
          ...
          <div className="md:w-56 lg:w-64 shrink-0 border-b md:border-b-0 md:border-r border-border bg-surface-muted/40 flex flex-col">
            ...
            <div className="p-2 space-y-0.5 overflow-y-auto flex-1">
              {(fleetVehicles ?? []).map((vehicle: FleetVehicle, i: number) => {
                ...
                const hasDbData = (dbPricing ?? []).some(
                  (p) => p.vehicleType === vehicle.name,
                );
                return (
                  <button
                    key={vehicle.id}
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span
                        ...
                        style={{
                          backgroundColor:
                            CAT_COLOR[vehicle.category] ??
                            "var(--color-text-muted)",
                        }}
                      />
                      <span
                        className={`text-sm truncate font-medium ${isActive ? "text-primary font-semibold" : "text-text-primary"}`}
                      >
                        {vehicle.name}
                      </span>
                      {!hasDbData && (
                    ...
                    <div className="flex items-center gap-1.5 mt-1 ml-4">
                      ...
                      <span
                        className={`text-[10px] font-medium ${isActive ? "text-primary/70" : "text-text-tertiary"}`}
                      >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      style={{
                        backgroundColor: `color-mix(in srgb, ${CAT_COLOR[v.category] ?? "var(--color-text-muted)"} 12%, transparent)`,
                        color:
                          CAT_COLOR[v.category] ??
                  ...
                  <p className="text-xs text-text-secondary mt-0.5">
                    {v.tagline ?? v.category} · {current.rows.length} service
                    fares configured
                  </p>
                </div>
                <Button
                  onClick={handleSave}
                  isDisabled={upsertPricing.isPending}
                  className={`flex items-center gap-2 transition-all shrink-0 shadow-sm disabled:opacity-60 ${
                    saved
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-3 items-center p-4 rounded-2xl bg-gradient-to-r from-primary/5 to-transparent border border-primary/15">
                    ...
                    <div className="w-36 shrink-0">
                      ...
                      <select
                        value={current.defaultUnit}
                        onChange={(e) =>
                          setDefault("defaultUnit", e.target.value as FareUnit)
                        }
                    ...
                    <div className="shrink-0 self-end pb-0.5">
                      <span
                        ...
                        style={{
                          backgroundColor: `color-mix(in srgb, ${UNIT_COLOR[current.defaultUnit] ?? "var(--color-text-muted)"} 12%, transparent)`,
                          color:
                            UNIT_COLOR[current.defaultUnit] ??
                            "var(--color-text-secondary)",
                        }}
                      >
                        {current.defaultUnit}
                ...
                <div>
                  ...
                  {current.rows.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 rounded-3xl border border-dashed border-border text-center">
                      <p className="text-sm font-semibold text-text-secondary">
                        No service fares yet
                      </p>
                      <p className="text-xs text-text-tertiary mt-1 mb-3">
                        Add fares for specific services like airport,
                        outstation, etc.
                      </p>
                      <button
                        onClick={addRow}
                    ...
                    <div className="space-y-2">
                      {current.rows.map((row, idx) => (
                        <div
                          key={idx}
                          className="group flex gap-3 items-start rounded-2xl border border-border bg-surface-muted/30 hover:bg-surface-muted/60 px-4 py-3 transition-colors"
                        >
