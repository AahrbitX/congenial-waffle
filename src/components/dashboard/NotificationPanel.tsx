...
import {
  IconX, IconBell, IconCar, IconWallet, IconGift, IconShield,
} from "@/constants/icons";
import type { NotificationType } from "@/types/notification.types";
import { ElementType } from "react";

const ICON_MAP: Record<NotificationType, { icon: ElementType; cls: string }> = {
  ride:    { icon: IconCar,    cls: "bg-[var(--color-primary-light)] text-[var(--color-primary)]"   },
  payment: { icon: IconWallet, cls: "bg-[var(--color-success-light)] text-[var(--color-success)]"   },
  promo:   { icon: IconGift,   cls: "bg-[var(--color-purple-light)] text-[var(--color-purple)]"      },
  safety:  { icon: IconShield, cls: "bg-[var(--color-danger-light)] text-[var(--color-danger)]"      },
  info:    { icon: IconBell,   cls: "bg-[var(--color-surface-muted)] text-[var(--color-text-secondary)]" },
};
...
export function NotificationPanel() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      <div className="flex-1 bg-black/30 backdrop-blur-sm" onClick={closeNotifPanel} />
      <div className="w-[390px] bg-[var(--color-surface)] h-full shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--color-border)]">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-[16px] font-black text-[var(--color-text-primary)]">Notifications</h2>
              {unread > 0 && (
            ...
            <p className="text-[12px] text-[var(--color-text-tertiary)] mt-0.5">Stay up to date</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={closeNotifPanel} className="p-1.5 rounded-lg hover:bg-[var(--color-surface-muted)]">
              <IconX size={16} />
            </button>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-[13px] text-[var(--color-text-primary)] ${!n.read ? "font-bold" : "font-semibold"}`}>
                          {n.title}
                        </p>
                        <p className="text-[12px] text-[var(--color-text-secondary)] leading-snug mt-0.5">{n.body}</p>
                        <p className="text-[11px] text-[var(--color-text-tertiary)] mt-1">{n.time}</p>
                      </div>
                    </div>
                  );
                })}
