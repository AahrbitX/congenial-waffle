...
const PAGE_META: Record<string, { title: string; subFn?: (name: string) => string }> = {
  [ROUTES.dashboard.overview]:              { title: "Overview",  subFn: (n) => `${greeting()}, ${n}` },
  [ROUTES.dashboard.rides]:                 { title: "My Rides",  subFn: () => "View and manage your trip history" },
  [ROUTES.dashboard.wallet]:                { title: "Wallet",    subFn: () => "Manage your balance and payments" },
  [ROUTES.dashboard.profile.root]:          { title: "Profile",   subFn: () => "Manage your account details" },
...
export function DashboardHeader() {
  ...
  return (
    <div className="flex items-center justify-between mb-6">
      <div>
        <h1 className="text-[22px] font-black text-[var(--color-text-primary)] tracking-tight">
          {meta.title}
        </h1>
        {meta.subFn && (
          <p className="text-[13px] text-[var(--color-text-tertiary)] mt-0.5">
            {meta.subFn(firstName)}
          </p>
        )}
      </div>
      <button
