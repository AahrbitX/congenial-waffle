...
function ChartSkeleton() {
  return (
    <ResponsiveCard>
      ...
      <Card.Content>
        <div className="h-[200px] flex items-end gap-4 px-2">
          {[70, 55, 45, 30].map((h, i) => (
            <div key={i} className="flex-1 flex items-end gap-1">
              <Skeleton className="flex-1 rounded-t" style={{ height: `${h}%` }} />
              <Skeleton className="flex-1 rounded-t" style={{ height: `${h * 0.9}%` }} />
            </div>
          ))}
        </div>
      </Card.Content>
...
export function VehiclePerformanceChart({ data, isLoading }: VehiclePerformanceChartProps) {
  ...
  return (
    <ResponsiveCard>
      <Card.Header>
        ...
        <Card.Description>Bookings and completed rides by vehicle category</Card.Description>
      </Card.Header>
