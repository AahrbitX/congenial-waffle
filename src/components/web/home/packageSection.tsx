...
function PackagesSection() {
  ...
  // Build map: serviceId → minimum fare amount across all vehicle types
  const serviceFareMap: Record<string, { amount: number; unit: string }> = {};
  for (const p of pricing ?? []) {
    for (const [sid, fare] of Object.entries(p.serviceFares ?? {})) {
      if (!serviceFareMap[sid] || fare.amount < serviceFareMap[sid].amount) {
        serviceFareMap[sid] = fare;
      }
    }
  }
    ...
    <section className="pb-10 px-6 sm:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {PACKAGES.map((pkg) => {
            const fare      = pkg.serviceId ? serviceFareMap[pkg.serviceId] : null;
            const priceText = fare ? `₹${fare.amount}` : pkg.serviceId ? "—" : "";
            const unitText  = fare ? fare.unit : pkg.unit;
            ...
            return (
              <Card key={pkg.name} className="overflow-hidden p-0 gap-0">
                ...
                <div className="p-4">
                  ...
                  <p className="font-medium mt-1.5 text-muted text-sm">{pkg.desc}</p>
